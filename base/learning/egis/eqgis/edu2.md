...
**以下も含めて、課題との関連などから、演習用データと解説用データは異なります。解説用データにはSRTMを使用しています。教材内では、merge-gsi-clipやmerge-srtm-clipのようなレイヤが登場しますが、両方とも標高を示すデータです。この教材では、`-gsi-`と`srtm`の表記のデータが入り混じって使われていますが、その都度読み替えて理解してください。重要なことはレイヤ名に関わらず、標高データを用いたデータ処理をしているということです。**
...
以下のように、ヒストグラムのタブから`ヒストグラムの計算`をすることで、ヒストグラムを確認できる。ヒストグラムは、標高ごとに色分けする際の参考にもなる。
![img](./img/edu2-9.png)
...
3. レイヤパネルで、標高値のラスタを選択（断面図を作りたいデータをレイヤパネル上でクリックし、青色のハイライトの状態にする）
4. `Add Layer`をクリック
